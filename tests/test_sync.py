...
class TestNexusSync(unittest.TestCase):
    ...
    def simulate_push(self, client: LocalRepo):
        ...
        for op in ops: op['payload'] = __import__('json').loads(op['payload'])
        
        if ops:
            self.hub.push_ops(ops)
            op_ids = [op['op_id'] for op in ops]
            placeholders = ','.join(['?'] * len(op_ids))
            client.conn.execute(f"UPDATE sync_op_log SET synced = 1 WHERE op_id IN ({placeholders})", op_ids)
            client.conn.commit()
    ...
    def simulate_pull(self, client: LocalRepo):
        ...
        for op in ops:
            ...
            if op['actor_id'] != client.actor_id:
                client.apply_to_db(op['entity_type'], op['entity_id'], op['action'], op['payload'])
    ...
    def test_field_level_merge(self):
        ...
        task_a = self.client_a.conn.execute("SELECT * FROM tasks WHERE id = ?", (task_id,)).fetchone()
    ...
    def test_markdown_conflict_resolution(self):
        ...
        self.client_a.record_local_op("NOTE", note_id, "UPDATE", {"content": "Versão do A"})
        self.client_b.record_local_op("NOTE", note_id, "UPDATE", {"content": "Versão do B"})
        ...
        self.assertTrue(os.path.exists(conflict_file), "O arquivo de conflito não foi gerado.")
        
        with open(main_file, 'r', encoding='utf-8') as f: self.assertEqual(f.read(), "Versão do B")
        with open(conflict_file, 'r', encoding='utf-8') as f: self.assertEqual(f.read(), "Versão do A")
