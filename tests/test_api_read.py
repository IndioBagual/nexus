...
class TestReadRepository(unittest.TestCase):
    def setUp(self):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY, title TEXT, priority TEXT, status TEXT, due_date TEXT)"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS expenses (id INTEGER PRIMARY KEY, amount REAL, category TEXT, created_at TEXT)"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS rpg_attributes (name TEXT, total_xp INTEGER, current_level INTEGER)"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS rpg_history (id INTEGER, description TEXT, xp_amount INTEGER, attribute TEXT, created_at TEXT)"
        )

        # Dados Mockados
        cursor.execute(
            "INSERT INTO tasks (title, priority, status, due_date) VALUES ('Task 1', 'high', 'TODO', '2026-01-01')"
        )
        cursor.execute(
            "INSERT INTO expenses (amount, category, created_at) VALUES (100, 'Food', '2030-01-01')"
        )
        cursor.execute(
            "INSERT INTO rpg_attributes (name, total_xp, current_level) VALUES ('STR', 100, 2)"
        )
