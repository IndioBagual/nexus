...
class SyncEngine:
    ...
    def push(self):
        ...
        try:
            res = requests.post(f"{self.hub_url}/sync/push", json={"operations": ops_to_push})
            if res.status_code == 200:
                # Marca como sincronizado localmente
                op_ids = [op['op_id'] for op in ops_to_push]
                placeholders = ','.join(['?'] * len(op_ids))
                self.repo.conn.execute(f"UPDATE sync_op_log SET synced = 1 WHERE op_id IN ({placeholders})", op_ids)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for op in ops:
                    ...
                    self.repo.apply_to_db(op['entity_type'], op['entity_id'], op['action'], op['payload'])
                    applied += 1
