...
class LocalRepo:
    ...
    def get_cursor(self):
        return self.conn.execute("SELECT last_cursor FROM sync_state WHERE id = 1").fetchone()[0]

    def set_cursor(self, cursor: int):
        self.conn.execute("UPDATE sync_state SET last_cursor = ? WHERE id = 1", (cursor,))
        self.conn.commit()

    def record_local_op(self, entity_type: str, entity_id: str, action: str, payload: dict):
        ...
        self.conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 0)
        """, (op_id, self.actor_id, entity_type, entity_id, action, json.dumps(payload), timestamp))
        self.conn.commit()
    ...
    def apply_to_db(self, entity_type: str, entity_id: str, action: str, payload: dict):
        ...
        if entity_type == "TASK":
            if action == "CREATE":
                cursor.execute("INSERT OR IGNORE INTO tasks (id, title, priority, status) VALUES (?, ?, ?, ?)",
                               (entity_id, payload.get('title'), payload.get('priority', 'medium'), 'TODO'))
            elif action == "UPDATE":
                # Merge em nível de campo: só atualiza os campos presentes no payload
                for key, value in payload.items():
                    if key in ['title', 'priority', 'status', 'due_date']:
                        cursor.execute(f"UPDATE tasks SET {key} = ? WHERE id = ?", (value, entity_id))
        
        elif entity_type == "EXPENSE":
            if action == "CREATE":
                cursor.execute("INSERT OR IGNORE INTO expenses (id, amount, category, description) VALUES (?, ?, ?, ?)",
                               (entity_id, payload.get('amount'), payload.get('category'), payload.get('description')))
            ...
            if action in ["CREATE", "UPDATE"]:
                if os.path.exists(file_path):
                    ...
                    if local_content != new_content:
                        conflict_path = os.path.join(self.notes_path, f"{entity_id}_conflict.md")
                        with open(conflict_path, 'w', encoding='utf-8') as f:
                            f.write(local_content) # Salva o local como conflito
                        print(f"⚠️ CONFLITO DETECTADO na nota {entity_id}. Merge manual necessário.")
