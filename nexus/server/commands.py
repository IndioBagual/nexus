...
from nexus.domain.entities import Task, Expense, Note
...
class NoOpCortex(CortexProvider):
    def parse_intent(self, text): return []
...
class CommandHandler:
    def __init__(self, db_path: str, notes_path: str):
        ...
        self.kernel = NexusKernel(
            self.sql_repo, self.sql_repo, self.fs_repo, self.sql_repo, NoOpCortex(), self.bus
        )
    ...
    def create_task(self, data: TaskCreate):
        ...
        self.kernel._on_task_created(type('Event', (object,), {'payload': {'id': tid, 'priority': task.priority}})())
        return {"id": tid, "status": "created"}

    def create_expense(self, data: ExpenseCreate):
        expense = Expense(amount=data.amount, category=data.category, description=data.description)
        eid = self.sql_repo.add(expense)
        self.kernel._on_expense_added(type('Event', (object,), {'payload': {'id': eid, 'amount': expense.amount}})())
    ...
    def create_note(self, data: NoteCreate):
        ...
        self.kernel._on_note_created(type('Event', (object,), {'payload': {'filepath': path}})())
