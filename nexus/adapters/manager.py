...
class AdapterManager:
    ...
    def get_cursor(self, adapter_id: str) -> str:
        conn = sqlite3.connect(self.db_path)
        row = conn.execute("SELECT last_cursor FROM adapter_cursors WHERE adapter_id = ?", (adapter_id,)).fetchone()
        conn.close()
    ...
    def run_sync_cycle(self):
        ...
        for adapter in self.adapters:
            ...
            context = NexusContext(self.db_path, adapter.adapter_id, adapter.required_permissions)
            ...
            try:
                ...
                if raw_data:
                    print(f"[{adapter.adapter_id}] {len(raw_data)} novos itens encontrados. Mapeando...")
