...
from nexus.domain.ports import ExpenseRepository, TaskRepository, NoteRepository, RPGRepository
...
class SQLiteAdapter(ExpenseRepository, TaskRepository, NoteRepository, RPGRepository):
    ...
    def _init_rpg_state(self):
        ...
        for attr in attributes:
            cursor.execute("INSERT OR IGNORE INTO rpg_attributes (name, total_xp, current_level) VALUES (?, 0, 1)", (attr,))
        self.conn.commit()
    ...
    def add(self, item) -> int:
        ...
        if isinstance(item, Expense):
            cursor.execute("INSERT INTO expenses (amount, category, description, currency) VALUES (?, ?, ?, ?)",
                           (item.amount, item.category, item.description, item.currency))
        elif isinstance(item, Task):
            cursor.execute("INSERT INTO tasks (title, priority, status, due_date) VALUES (?, ?, ?, ?)",
    ...
    def get_attribute(self, name: str) -> RPGAttribute:
        cursor = self.conn.cursor()
        cursor.execute("SELECT name, total_xp, current_level FROM rpg_attributes WHERE name = ?", (name,))
        row = cursor.fetchone()
        if row:
    ...
    def update_attribute(self, attr: RPGAttribute):
        cursor = self.conn.cursor()
        cursor.execute("UPDATE rpg_attributes SET total_xp = ?, current_level = ?, last_updated = CURRENT_TIMESTAMP WHERE name = ?", 
                       (attr.total_xp, attr.current_level, attr.name))
        self.conn.commit()

    def log_xp_history(self, attribute: str, amount: int, source: str, event_id: str, desc: str):
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO rpg_history (attribute, xp_amount, source_type, source_id, description) VALUES (?, ?, ?, ?, ?)",
