...
class NexusKernel:
        ...
        for intent in intents:
            ...
            try:
                if action == "add_expense":
                    ...
                    self.bus.publish(
                        Event(
                            EVENT_EXPENSE_ADDED, {"id": eid, "amount": expense.amount}
                        )
                    )
                    results.append(
                        f"💸 [TREASURY] R${expense.amount} em '{expense.category}': {expense.description}"
                    )
                    ...
                    self.bus.publish(
                        Event(
                            EVENT_TASK_CREATED, {"id": tid, "priority": task.priority}
                        )
                    )
                    results.append(
                        f"✅ [CHRONOS] Task '{task.title}' para {task.due_date or 'Hoje'}"
                    )
                    ...
                    self.bus.publish(
                        Event(EVENT_NOTE_CREATED, {"filepath": path, "tags": note.tags})
                    )
                    ...
                    attr_name = params.get("attribute") or attr_map.get(
                        action_type, "WIS"
                    )
                    ...
                    results.append(
                        f"⚔️ [RPG] Ação manual registrada: {params.get('description')}"
                    )
    ...
    def _apply_xp(
        ...
        for evt in events:
            if evt.name == EVENT_RPG_XP_GAINED:
                print(
                    f"✨ [RPG] +{evt.payload['amount']} {evt.payload['attribute']} (Total: {evt.payload['total_xp']})"
                )
            elif evt.name == EVENT_RPG_LEVEL_UP:
                print(
                    f"🎉 [RPG] LEVEL UP! {evt.payload['attribute']} alcançou Nível {evt.payload['new_level']}!"
                )
    ...
    def _on_expense_added(self, event: Event):
        ...
        self._apply_xp(
            "WIS", 5, "TREASURY", str(event.payload["id"]), "Despesa registrada"
        )
    ...
    def _on_task_created(self, event: Event):
        ...
        self._apply_xp(
            "WIS", xp, "CHRONOS", str(event.payload["id"]), "Tarefa planejada"
        )

    def _on_note_created(self, event: Event):
        # Regra: Notas aumentam Intelecto (INT)
        self._apply_xp(
            "INT", 10, "LIBRARY", event.payload["filepath"], "Conhecimento capturado"
        )
