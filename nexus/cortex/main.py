...
def cleanup_expired_proposals():
    ...
    try:
        ...
        cutoff_date = (datetime.datetime.utcnow() - datetime.timedelta(hours=48)).isoformat() + "Z"
        ...
        conn.commit()
        if deleted_count > 0:
            print(f"🧹 [TTL] Limpeza: {deleted_count} propostas expiradas foram removidas da fila.")
...
def ingest(req: IngestRequest):
    ...
    if os.path.exists(notes_path):
        for f in os.listdir(notes_path):
            if f.endswith(".md"):
                with open(os.path.join(notes_path, f), "r", encoding="utf-8") as file:
...
def approve_proposal(proposal_id: str):
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.execute("SELECT * FROM proposed_actions WHERE id = ? AND status = 'PENDING'", (proposal_id,))
        ...
        conn.execute("UPDATE proposed_actions SET status = 'APPROVED' WHERE id = ?", (proposal_id,))
        conn.commit()
...
def reject_proposal(proposal_id: str):
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("UPDATE proposed_actions SET status = 'REJECTED' WHERE id = ?", (proposal_id,))
        conn.commit()
...
def get_chronos_today():
    return {
        "today": [
            {"id": "t1", "title": "Revisar arquitetura do NEXUS", "status": "TODO", "priority": "high", "due_date": None},
            {"id": "t2", "title": "Treino físico (30 min)", "status": "TODO", "priority": "medium", "due_date": None},
            {"id": "t3", "title": "Ler 10 páginas", "status": "DONE", "priority": "low", "due_date": None}
        ],
        "overdue_count": 1
    }
...
def get_chronos_tasks(status: str = "open"):
    if status == "open":
        tasks = [
            {"id": "t1", "title": "Revisar arquitetura do NEXUS", "status": "TODO", "priority": "high", "due_date": "Hoje"},
            {"id": "t2", "title": "Treino físico (30 min)", "status": "TODO", "priority": "medium", "due_date": "Hoje"}
        ]
    else:
        tasks = [
            {"id": "t3", "title": "Ler 10 páginas", "status": "DONE", "priority": "low", "due_date": "Ontem"}
        ]
...
def get_treasury_summary(range: str = "week"):
    ...
    return {
        ...
        "recent_transactions": [
            {"id": "tx1", "amount": 45.90, "type": "EXPENSE", "category": "Ifood", "date": "2026-02-20"},
            {"id": "tx4", "amount": 120.00, "type": "EXPENSE", "category": "Mercado", "date": "2026-02-18"},
            {"id": "tx2", "amount": 2500.00, "type": "INCOME", "category": "Pagamento", "date": "2026-02-15"}
        ]
    }
...
def get_library_notes(limit: int = 5, offset: int = 0):
    return {
        "notes": [
            {"id": "n1", "title": "Cibersegurança: Descoberta de IPs", "tags": ["tech", "aula"], "last_modified": "2026-02-19"},
            {"id": "n2", "title": "Ideias para o App de Flert", "tags": ["projetos", "web"], "last_modified": "2026-02-15"},
            {"id": "n3", "title": "Saúde Mental: Estrutura da Apresentação", "tags": ["psicologia", "palestra"], "last_modified": "2026-02-10"}
        ],
        "total_count": 3
    }
...
def search_library(q: str):
    ...
    filtered = [
        ...
        if q_lower in n["title"].lower() or any(q_lower in tag.lower() for tag in n["tags"])
    ]
